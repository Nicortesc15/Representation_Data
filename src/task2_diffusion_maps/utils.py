...
def create_distance_matrix(X, max_distance=200):
    ...
    tree = KDTree(
        X
    )  # Create a tree to look up the nearest neighbors of a point

    for i in range(N):
        radius_points = tree.query_ball_point(
            X[i], max_distance
        )  # Row index of the points inside the radius
        for j in radius_points:
            if i != j:
                distace = np.linalg.norm(
                    X[i] - X[j]
...
def reconstruct_data_using_truncated_svd(
    ...
    """
    # Obtain the r columns of U corresponding to largest singular values
    U_r = U[:, :n_components]

    # Create a diagonal matrix with the first r singular values from S
